...
function guppy_init(path){
    guppy_get_latexify(path);
}


function guppy_is_blank(n){
    return n.firstChild == null || n.firstChild.nodeValue == '';
}

function guppy_get_latexify(xsl_path){
...
function guppy_mouse_down(e){
    console.log(e);
    var n = e.target;
    if(e.target == document.getElementById("toggle_ref")) toggle_div("help_card");
    else while(n != null){
	if(n.id in GUPPY_INSTANCES){
	    active_guppy = GUPPY_INSTANCES[n.id];
	    active_guppy.activate();
	    return;
	}
	n = n.parentNode;
    }
    if(active_guppy != null){
	active_guppy.deactivate();
	active_guppy = null;
    }
}
...
window.addEventListener("mousedown",guppy_mouse_down, false);
window.addEventListener("focus", function(e) { alt_down = false; }, false);

var Guppy = function(div_id, properties){
    if(div_id in GUPPY_INSTANCES){
	console.log("There is already a Guppy object with this ID");
	return;
    }
    GUPPY_INSTANCES[div_id] = this;
    ...
    this.sel_start = null;
    this.sel_end = null;
    this.editor = document.getElementById(div_id);
    ...
    this.sel_status = SEL_NONE;
    
    // this.render();
    this.checkpoint();
    if(active_guppy == null) active_guppy = this;
    console.log("ACTIVE",active_guppy);
}

Guppy.prototype.content = function(t){
    if(t != "xml") return guppy_xsltify(t,this.base);
...
Guppy.prototype.render_node = function(n,t){
	...
	var cleanup = [];
	var sel_cursor;
	if(this.sel_status == SEL_CURSOR_AT_START) sel_cursor = this.sel_end;
	if(this.sel_status == SEL_CURSOR_AT_END) sel_cursor = this.sel_start;
	// Add cursor
	...
	if(this.current.firstChild.nodeValue != "" || this.current.previousSibling != null || this.current.nextSibling != null){
	    ...
	    var idx = this.caret;
	    if(this.sel_status == SEL_CURSOR_AT_START) caret_text = caret_text + "\\color{"+SEL_COLOR+"}{";
	    if(this.sel_status == SEL_CURSOR_AT_END) caret_text = "}" + caret_text;
	    //if(this.sel_status == SEL_CURSOR_AT_END && sel_cursor.node == current) idx += SEL_CARET.length;
	...
	// Add sel_start
	if(this.sel_status != SEL_NONE) {
	    var idx = sel_cursor.caret;
	    if(sel_cursor.node == this.current){
		if(this.sel_status == SEL_CURSOR_AT_START) idx += caret_text.length;
	    ...
	    if(this.sel_status == SEL_CURSOR_AT_END) sel_caret_text = sel_caret_text + "\\color{"+SEL_COLOR+"}{";
	    if(this.sel_status == SEL_CURSOR_AT_START) sel_caret_text = "}" + sel_caret_text;
	...
	// Render: 
	output = guppy_xsltify(t, this.base);
...
Guppy.prototype.sel_right = function(){
    if(this.sel_status == SEL_NONE){
	this.set_sel_start();
	this.sel_status = SEL_CURSOR_AT_END;
    }
    console.log("EEEE");
	...
	if(nn != null){
	    ...
	    this.caret = 0;
	    this.set_sel_boundary(SEL_CURSOR_AT_END);
	    console.log("asda");
	}
...
Guppy.prototype.set_sel_boundary = function(sstatus){
    if(this.sel_status == SEL_NONE) this.sel_status = sstatus;
    if(this.sel_status == SEL_CURSOR_AT_START)
	this.set_sel_start();
    else if(this.sel_status == SEL_CURSOR_AT_END)
	this.set_sel_end();
}
...
Guppy.prototype.sel_left = function(){
    if(this.sel_status == SEL_NONE){
	this.set_sel_end();
	this.sel_status = SEL_CURSOR_AT_START;
    }
    console.log("EEEE");
    if(this.caret <= 0){
	...
	if(nn != null){
	    ...
	    this.set_sel_boundary(SEL_CURSOR_AT_START);
	    console.log("asdeee");
	}
...
Guppy.prototype.get_length = function(n){
    if(guppy_is_blank(n) || n.nodeName == 'f') return 0
