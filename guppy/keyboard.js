...
function is_blacklisted(symb_type){
    for(var i = 0; i < Guppy.active_guppy.type_blacklist.length; i++)
	if(symb_type == Guppy.active_guppy.type_blacklist[i]) return true;
    return false;
}
...
function key_down(e){
	...
	if(keycode == UP){ Guppy.active_guppy.insert_symbol(Guppy.active_guppy.current,Guppy.active_guppy.caret,"exp"); }
	else if(keycode == DOWN){ Guppy.active_guppy.insert_symbol(Guppy.active_guppy.current,Guppy.active_guppy.caret,"sub"); }
	else if(keycode == LEFT){ Guppy.active_guppy.sel_left(); }
	else if(keycode == RIGHT){ Guppy.active_guppy.sel_right(); }
	else if(keycode == RPAREN){ console.log("RP"); Guppy.active_guppy.right_paren(); }
	else if(keycode in sk_chars){ Guppy.active_guppy.node_insert(sk_chars[keycode]); }
	else if(keycode in sk_syms){ Guppy.active_guppy.insert_symbol(Guppy.active_guppy.current,Guppy.active_guppy.caret,sk_syms[keycode]); }
	else if(65 <= e.keyCode && e.keyCode <= 90){ Guppy.active_guppy.node_insert(String.fromCharCode(e.keyCode)); }
    }
    else if(!alt_down){
	e.returnValue = false; e.preventDefault(); 
	console.log(e.keyCode,e.srcElement);
	if(keycode == UP){ e.returnValue = false; e.preventDefault(); Guppy.active_guppy.up(); }
	else if(keycode == DOWN){ e.returnValue = false; e.preventDefault(); Guppy.active_guppy.down(); }
	else if(keycode == LEFT){ e.returnValue = false; e.preventDefault(); Guppy.active_guppy.left(); }
	else if(keycode == RIGHT){ e.returnValue = false; e.preventDefault(); Guppy.active_guppy.right(); }
	else if(keycode == HOME){ e.returnValue = false; e.preventDefault(); Guppy.active_guppy.home(); }
	else if(keycode == END){ e.returnValue = false; e.preventDefault(); Guppy.active_guppy.end(); }
	else if(keycode == BACKSPACE){ e.returnValue = false; e.preventDefault(); console.log("AA"); Guppy.active_guppy.backspace(); }
	else if(keycode == 16) shift_down = true;
	else if(keycode == 17) ctrl_down = true;
	else if(keycode == 18) alt_down = true;
	else if(keycode in k_chars){ Guppy.active_guppy.node_insert(k_chars[keycode]); }
	else if(keycode in k_syms){ Guppy.active_guppy.insert_symbol(Guppy.active_guppy.current,Guppy.active_guppy.caret,k_syms[keycode]); }
