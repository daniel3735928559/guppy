...
var buildExpression = function(expression, options, prev) {
    ...
    for (var i = 0; i < expression.length; i++) {
        ...
        groups.push(buildGroup(group, options, prev, prevprev)); // HAX
	prevprev = prev; // HAX
        prev = group;
    }
    return groups;
...
var groupTypes = {
    ...
    bin: function(group, options, prev) {
        ...
        if (!prev || utils.contains(["mbin", "mopen", "mrel", "mop", "mpunct", "cursor"],
    ...
    rule: function(group, options, prev) {
        ...
        var rule = makeSpan(["cursor", "rule"], [], options.getColor()); // HAX
	...
	rule.style.marginLeft = "0px";             // HAX
	rule.style.marginRight = "-1px";             // HAX
	rule.style.borderLeftWidth = "1px";          // HAX

        rule.style.borderRightWidth = "0px"; // HAX
 ...
 */
var buildGroup = function(group, options, prev, prevprev) { // HAX
    if (!group) {
        return makeSpan();
    }
	...
	if(prev && prev.value && prev.value.value && prev.value.value[0] && prev.value.value[0].type == "rule"){ // HAX
	    prev = prevprev; // HAX
	} // HAX
        var groupNode = groupTypes[group.type](group, options, prev, prevprev);
        var multiplier;
